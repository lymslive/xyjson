...
## TODO:2025-11-05/2 增加一些 example 示例应用

需要增加 example ，不依赖测试框架可直接运行的示例。

基本需求：
- 增加 exmaple/ 子目录
- example 内每个 .cpp 文件是一个独立示例，能分别编译为可执行目标
- example 内增加 CMake 构建脚本，在上层根目录添加子目录
- 示例程序尽量用操作符表达式，但如果用方法名更符合直觉也可使用。

我先提供两示例思路，你再帮我补充一个。

### 示例一：jp 
极简版 jq ，根据路径查询打印 json 的子结点。

基本用法：./jp /path/to/file.json?/path/to/node

接收一个参数，用 `?` 分隔参数为两部分，前面是 json 文件的路径，后面是在 json
树内的路径。先解析 json 文件，如果解析错误，打印错误消息，退出。再查询路径结点
，如果路径不存在，打印警告消息，否则将结点默认按单行格式打印。

如果省去 `?` 前面部分，或用 `-` 代替，即以参数以 `?` 或 `-?` 开头，表示从标准
输入读入一个完整的 json 内容。

如果参数不包含 `?` ，则表示 json 文件路径，结点路径为空表示取根结点。可用于检
测 json 格式正确性与压缩打印 json 。

### 示例二：C++ 简单结构体与 Json 的映射

写一个示例，定义一个结构体包含 5-10 个字段，包含数字、字符串、bool 等基本类型
。为该结构体定义一个方法从 yyjson::Value 读取每个字段，一个方法将每个字段写入
yyjson::MutableValue 对象。Json 字段名与结构体字段名保持一致。

请展示这种用法。编译程序可直接运行，用默认的 json 演示基本用法，也可接收命令参
数表示的 json 文件，或从标准输入读取 json 内容。程序反序列化解析，转为结构体做
一些简单业务处理，再序列化输出。

### 示例三：
请你帮忙再想个应用示例，能较好地演示 xyjson 的特点。

### DONE: 20251105-140215

## TODO:2025-11-05/3 研究可变迭代器初始化要不要 next 一步

解引用要预取 next 指针，`++` 调next() 方法时再移动 next 指针.
在循环迭代中似乎有重复取 next 指针的操作。迭代器本就为了性能而设计，是否值得考
虑优化这个重复操作，有多少性能影响。

在 C 的原生迭代器中，每次调用 `_next()` 方法获取当前值，并将迭代器的当前指针沿
next 指针前移，取值与前进是同时进行的，这没问题，内部如何实现都可以。但在 C++
迭代器中，按标准循环写法，`++` 前进与 `*` 解引用取值是分开的两次调用，那就产生
了重复取 next 指针的问题。

这个问题其实也源于 `m_iter.cur` 不是真正的当前指针，实际上是前一个指针，它初始
化为环形链表的尾结点，必须往前看一步才是读取当前结点。

如果在 C++ 迭代器构造时就将当前指针前进一步，是否更合理的设计？请仔细分析一下
若作此改动，有哪些影响？哪些方法需要做相应调整。

可渐进重构，先评估可变数组迭代器的调整，可行的话再类似调整可变对象迭代器。

xyjson 调用的 C api 函数实现，在 yyjson.h 中都能找到，可深入分析对比一下。
...
## TODO:2025-11-06/2 使用条件编译宏控制部分功能开启

`MutableValue::m_pendingKey` 成员变量的引入，是为了支持 `<< key << value` 的链
式使用方式。代价是增加了 1/3 的内存布局。增加一个宏，在有定义时屏蔽该功能。

此外，可写模型及相应的可写迭代器，代码量多，如果用户没有可写的需求，只想用只读
模型，也允许通过宏屏蔽大量与可写相关的代码，缩短编译时间。为了不使条件编译宏不
把代码分隔得太破碎，分析是否允许可写类的前向声明始终对编译器可见，主要控制
Part 2 与 Part 4 Part 5 与可写类相关的部分代码。

参考 yyjson 对功能开启的宏定义名风格，用 `XYJSON_DISABLE_` 作为前缀。也可在
Part 1 对这类宏作前向声明，`#ifndef` 与 `#endif` 之间不必包含任何代码，仅为在
其前面加上注释之用。

然后在 CMake 构建脚本中再增加一个 miniut 小测试目标，为其定义以上两个控制宏，
只加入 `t_basic.cpp` 与 `t_experiment.cpp` 两个测试文件进行编译链接。
其中 `basic_size` 测试用例名断言了 MutableValue 类的大小，需要根据功能禁用宏
调整期望中。miniut 若有发现其他测试失败或编译问题，可能也需要加相应的宏调整。

保持 utxyjson 测试程序默认开启所有功能，用于完整的功能回归测试。
...
## TODO:2025-11-07/1 如何做 xyjson 的性能测试

先定个小目标，我想知道用 xyjson 封装为 C++ 后，与使用 yyjson 原生 C API 相比有
多少差距；在哪些场景下性能下降较大，又是否有些场景使用 C++ 反而更快些。

至于 yyjson 与其他 json 库的性能比较，则是 yyjson 库该关注的事，不是 xyjson 的
主要担心的事。

目前本项目使用自研的 couttast 单元测试框架做功能与回归测试。其实它在运行每个用
例时也会打印所耗的微秒数，可大致看一下运行一个示例要跑多久。但要做更完善的性能
测试可能需要额外调整工作。

请查阅本地安装的 ~/include/couttast/tinytast.hpp ，它也提供了一些简单的为性能
测试的宏，比如 `TIME_TAST` 循环调用另一个测试用例多次记时。所以请分析一下就用
这个库能否做好性能测试。与其他流行的性能测试框架相比有哪些不足。

假如也用 couttast 库先做些基础性能测试或性能对比。我想应该与 utxyjson 目标分开
编译为另一个可执行执行程序为好。将为性能测试的代码单独建个子目录，像 example
那样在 CMake 中添加子目录。
...
## TODO:2025-11-10/1 文档示例代码管理同步单元测试

- 先针对文档：READE.md docs/usage.md
- 在代码示例之前加隐藏注释 example:name ，其中 name 遵循单元测试用例名，加
  `readme_` 或 `usage_` 前缀。
- 增加单元测试文档 `utest/t_readme.cpp` 与 `utest/t_usage.cpp` 收集对应文档中的
  示例，为每个示例定义一个 `DEF_TAST` 测试用例名。
- 拷入的示例代码放在一对 `{}` 作用域中，前面加 `DESC("docs example:")` 注释宏；
- 在拷入块前可以添加必要的初始变量定义，以符合语法；
- 在拷入块中，凡 `std::cout` 输出语句用特殊注释 `//+` 标记，然后替换为 `COUT`
  断言语句；将来要考虑同步回文档时要删除 `COUT` 宏，恢复 `std::cout` 语句;
- 在拷入块后可以再添加一些 `COUT` 语句断言示例的变量值符合预期输出。
- 编译检查示例语法正确，运行检验行为正确性，作必要修改调整；
- 可在文件级补上 `using namespace yyjson` ，及 `#include` 必要头文件。

补充需求：usage.md 文档较长，请先为各级标题编号，从二级标题开始编号加 `## 1 `，
`### 1.1 ` 之类的编号前缀。然后为了方便文档内示例名唯一不易冲突，也可以在示例
名中带上编号中缀，完整示例名类似 `example:usage_1_1_xxx`，其中 `:` 后面部分与
`t_usage.cpp` 中 `DEF_TAST` 定义的用例名相同。

readme.md 的示例文档较少，先不必编号标题。

### 需求变更

不用 `DESC("docs example:")` 注释宏加 `{}` 缩进块。
改为用条件编译宏 `#ifdef MARKDOWN_CODE_SNIPPET` / `#endif` 
来对应从文档中拷入的代码片断。

从文档中提取的测试用例改为编译为单独的 utdocs ，不与主测试 utxyjson 混在一起。

### DONE: 20251110-105841

## TODO:2025-11-10/2 文档示例代码优化及格式调整

//+ 特殊注释标记顶格写。用两个正则完成。

在循环重名键的示例中 `it %= "two"` 会限入死循环，文档理解有误，始终重定位在第
一个键，删除去该示例及文档说明。

手动校验一遍文档的测试用例。

## TODO:2025-11-11/1 优化文档示例代码反向更新文档

在 script/ 目录下添加一个 perl 脚本，支持将 `t_readme.cpp` 与 `t_usage.cpp `中
的测试用例代码反向同步到文档 `README.md` 与 `usage.md` 中。

测试与示例代码对应规则如下：
- `DEF_TAST` 用例名对应文档注释标记 `<!-- example: -->`
- 提取条件编译宏 `#ifdef MARKDOWN_CODE_SNIPPET` 与 `#endif` 包围起来的代码块，
  包括注释行与中间的空行，插回文档的示例代码片断中
- 代码块中的 `COUT|DESC|CODE` 等宏语句删除，只应在测试文件中起作用，不要暴露到文档中；
- 代码块中的特殊注释 `//+ ` 行在文档中要取消注释，需要在文档中恢复为正常语句；
- 压缩代码块末尾的空行，可能删除宏语句后会留下多余的空行

在正式修改文档之前，先检查每个示例是否真的有更新，如果没有任何更新，就没必要修
改文档文件了。如果发生了修改，报告更新了几个用例。只修改能匹配用例名的示例，不
删除文档中的示例代码或增加新示例，但需要报告两边不匹配的用例名。文档中少量特殊
标记为 `example:NO_TEST` 的示例，不必在测试文件中有对应。

测试脚本：运行修改文档后，用 `git diff usage.md` 对比。当前应该只发生少量有意
增加的测试注释行的变更。脚本的用途是将来优化测试示例后，能一键更新文档的示例。

### DONE: 20251111-152917

## TODO:2025-11-11/2 api.md 文档重写方案

docs/api.md 文档需要全面调整优化。
...
核心类介绍部分：当前列表中 `xyjson::` 的命名空间前缀是错的，应该是 `yyjson::`
，但是列表行文宜简洁，可省去命名空间，只在文档最前面说明命名空间。然后加一个类
图，用 plantuml 语法表示。类图单独存文件，在文档中加链接。

操作符部分：
- 总览表按优先级从上到下排列
- 加上 `.` 操作符，注明该操作符不可重载，调用方法，放在表中展示相对优先级
- 字面量操作符 `_xyjson` 也列入表中，它的优先级怎么算？
- 然后三级标题也按优先级介绍每一个操作符重载，有多重重载意义的可再加四级标题
- 示例代码，操作符 api 与对应的方法名 api 用法放在同一个示例代码块中，但分成前
  后两个 `{}` 作用域，并加上 `/* use operator */` 与 `/* use method */` 的注释。
  示例要求完整，将来能拷入单元测试编译运行。在 `{}` 外层作用域，可以定义共用的
  json 文档变量。
- Document 转调 root 的同义操作符，在最后单独列一小节说明；在前面着重以 Value
  类为例介绍操作符，不必重复 Document 用例。但是一些操作符在  Document 类有不同
  意义的，也要在前面按优先级时专门介绍。'

类型常量部分：
就是 kInt kString kObject 等常量，说明可用在 `&` 与 `|` 操作符右侧参数。在前面
介绍这些操作符时，参数使用字面量 `0` 或 `""` 等。

泛型名称约定，在文档行文中，可以约定如下名称泛指类型：
- jsonT: Value 与 MutableValue 类
- docT: Document 与 MutableDocument 类
- iteratorT: 四种迭代器类
- scalarT: 数字与字符串等基本类型

在讲解各个操作符时，在抛出示例之前要简叙语法，两侧参数的类型，返回值的类型。
可参考 uasge.md 文档的风格。但是 api.md 假定读者不会从头到尾阅读，而是在需要时
查询个别章节参考。

### DONE: 20251111-165732

## TODO:2025-11-12/1 api.md 文档校对及优化

操作符总览表单独保存在 docs/operator.csv ，已校对修改过。需要再重新转为
markdown 格式更新到 docs/api.md 文档中。
已用 `<!-- begin operator table -->` 与 `<!-- end operator table -->` 注释标记
了表格位置。

然后 `## 操作符重载` 二级标题下面总览表后面的各三级标题，需要按总览表的优先级
顺序重新调整。相同操作符有多重意义的，在表中也分别列出多行，后文再按划分四级标
题。例如 `~` 操作符安排三级标题，下面 `~doc` 与 `~it` 再分四级标题。三级标题中
使用常见的标准的命名如 `按位非 ~` ，四级标题再根据我们的重载语义重命名。

我已经修改了前两个操作符的文档，请在 `<!-- refine line -->` 之后开始修改，调整
顺序及优化内容。另外注意表中有少量操作符目前在 `xyjson.h` 暂不支持，如 `--it`
与 `(int) json` ，可以写先文档，后面再补提需求实现。

### 补充需求

需要继续优化修改 api 文档。范围 `## 操作符重载` 二级章节中 `<!-- refine line -->`
后面的部分，请参考这之前的风格修改这之后的小节：

- 三级标题用 C++ 惯用常见的操作名称，符号写在名称后面
- 四级标题述重载意义，后接对应的方法名，不带括号
- 即使一个操作符只有一个重载意义，也要加一个四级标题，将来方便浏览目录
- 语法、功能、参数类型介绍下移到四级小节
- 一个四级小节内有必要时允许多个代码示例片断
- 示例中开始的 `/* use operator */` 可删除，后面`/* use method */` 注释附加
  `:` 与实际对应的方法带括号。涉及相同的变量名，加 `{}` 开启子作用域，而不是
  加 `2` 后缀区分。
- Document 转调 root 的操作符示例用法，统一移到后面的专门的三级小节

### DONE:20251112-171645

## TODO:2025-11-12/2 为迭代器补充 `--` 操作符

为了与 `++it` 对应，实现 `--it` 与  `it--`。
...
请在两个 Value 中定义 explicit 类型转换函数，以支持这种用法。

另外在 Document 类增加 explicit 构造函数，接收另一种 Document 类作为参数，实现
类型 `~` 的转换功能。以便支持如下用法：
...
## TODO:2025-11-14/1 优化更新文档表格的脚本

当前已有 script/utable.pl 脚本用于更新 `utest/README.md` 文档末尾的测试用例表
。但是该脚本逻辑太僵硬，希望能拆分为可能通过管道连接、读写标准输入输出的脚本，
增加复用与扩展性，以及便于分步调试。

- 开发 tsv2mdtable.pl 脚本将制表符 `\t` 分隔的文本转 mardown 格式表格
- 开发 csv2mdtable.pl 脚本将 `.csv` 格式的文本转 mardown 格式表格
- 开发 update_par.pl 脚本文档的一个段落，支持指定起始行模式
- 更新根目录 ./makefile 文件将 utable 目标改为调用 `script/utable.sh`
- 根目录 ./makefile 文件增加 otable 目标调用 `script/otable.sh`

其中 `utable.sh` 与 `otable.sh` 脚本已写好，就是简单调用组合调用以上几个 perl
脚本完成具体工作，因为管道组合命令行较长，单独封装在 bash 脚本中方便调用，也是
perl 的使用示例。

`tsv2mdtable.pl` 与 `csv2mdtable.pl` 脚本逻辑类似，分别用于将 `\t` 或 `,` 分隔
的文件行转为 markdown 格式的表格。允许可选参数 `--head=` 指定表头行，如果未指
定该参数，就假设输入的第一行是表头。转换格式后的文本打印至标准输出。其输入支持
从标准输入或文件名命令行参数，perl 的 `<>` 操作符可自动支持这两种情况，但要注
意如果用户输入了 `--head=` 等选项参数时，要将这些参数移除，剩下的认为是文件名
参数。

`update_par.pl` 脚本支持 `--begin=` 与 `--end=` 参数分别指定文档中要更新的段落
区域的起始行与结束行标记，支持正则表达式。起始与结束标记行保留原样不修改。如果
省略 `--begin=` 或 `--end=` 参数，默认是空行，找到第一个空行与第二个空行，将标
准输入的内容替换这两行中间的内容，前后的空行也是一样的逻辑保留原样。也支持
`--input=` 选项参数指定文件名表示输入内容，默认是从标准输入读取更新内容。然后
第一个位置参数是待更新的文件名。更新前先判断待更新区域的内容是否也输入完全一样
，如果一样的话就没必要更新了，不一样才要更新，更新前先备份。

这三个 perl 脚本都允许打印一些提示信息或调试信息至标准错误，要与标准输出分开。
用户不想看调试信息时可以用 `2>/dev/null` 过滤。

### DONE: 20251114-175941

## TODO:2025-11-17/1 开发更新 markdown 文档标题的脚本

参考之前 script/otable.sh 更新表格的管道分步办法，再开发两个 perl 脚本用于更新
文档的编号标题。

- mdtitle_order.pl: 提取一个 .md 文档的标题行，并编号，打印标准输出
- mdtitle_update.pl: 更新一个 .md 文档的标题，从标准输入读取标题行
- reorder_title.sh: 接收一个 .md 文件名为参数，管道组合前两个脚本更新标题

mdtitle_order.pl 接收参数：
- `--level=2` 从几级标题开始编号，默认是二级标题即 `##` 开头的标题；
- `--depth=4` 最多为几层标题编号，默认 4
- `--help` 打印帮助信息
- 位置参数：文件名或从标准输入读取源文档，可用 perl 的 `<>` 操作自动处理
...
如果 `--level=0` 表示标题不编号，原有编号须删除。大于 0 时对原标题重新编号。

mdtitle_update.pl 接收参数：
- `--target=file`：待更新的目标文件名
- `--help`：打印帮助信息
- 位置参数：文件名或从标准输入读入标题行，可用 perl 的 `<>` 操作自动处理

mdtitle_update.pl 更新标题的核心逻辑，不改变目标文档既有的层次结构，只更新对应
层级的标题。如果某层级标题的输入比目标文档少，目标文档多余的标题保留原样不更新；
如果某层级标题的输入比目标文档多，可以在该层级末尾增加标题行，相当于给目标文档
增加标题提纲，留待以后补充内容。

简单合并算法：
逐行比对 @target_lines 与 @title_lines ，做一个类似合并数组的操作。只不过
@target_lines 中有许多非标题行须原样输出，能与 @title_lines 匹配的行取
@title_lines 的更新，不匹配的标题行，不管来自哪个数组，都原样输出。

mdtitle_update.pl 在更新标题、覆盖目标文件之前先检测，如果实际上没有任何改动
，则不必更新；否则需要更新前先备份。

利用该脚本更新 docs/api.md 并测试，管道组合使用方法大致类似：

```bash
cat docs/api.md | script/mdtitle_order.pl | script/mdtitle_update.pl --target=docs/api.md
```

将该管道用法长命令行写入 reorder_title.sh 脚本，接收一个文件名参数。

### DONE: 20251117-095156
...
## TODO:2025-11-17/3 优化 markdown 文档标题重编号功能

script/mdtitle_order.pl 最初的需求是只打印标题行。现在再加一个选项，可控制是否
也打印其他非标题行。

增加选项 `--toc` 只打印标题行，就相当于原来的脚本功能。默认行为是原样输出非标
题行，打印重编号了标题的完整文件至标准输出，客户可重定向保存。

该选项也支持参数，没有参数时 `--toc` 或 `--toc=` 相当于 `--toc=0` ，对重编号的
标题行不做额外修饰。如果 `--toc=-1` ，打印 "行号\t标题"，先打印行号再打印标题
。如果 `--toc=n` 大于 0，先打印标题再打印行号，参数 `n` 表示从第几列开始打印行
号，中间补充空格；如果某个标题行长度大于 `n` ，不截断标题，只隔一个空格打印行
号。

须特殊处理中文。一个汉字的显示宽度按 2 个英文字符计算。

原来已用 Getopt::Long 处理命令行参数，它能否处理 `--toc` 这样既支持无参数，又
支持带参数的情况？如果不支持，可降级处理，但要说明用法。

再同步更新 reorder_title.sh 脚本，加上 `--toc` 选项只输出标题行。

评估一下修改后的 mdtitle_order.pl 是否能独立支持更新文档编号，类似如下用法：
```bash
cp doc.md doc.md.bak
cat doc.md.bak | mdtitle_order.pl > doc.md
```

不过这样必然会发生备份与覆盖重写目标文件的事件，原来结合管道至
mdtitle_update.pl 可检测更新，只在必要时备份重写。而且 mdtitle_update.pl 还支
持手动编辑单独的纯标题行再更新目标文档。
...
- 为 api.md 文档的每个示例添加注释标记 `<!-- example:api_章节编号_用例名 -->`
- 将每个用例拷到 utest/t_api.cpp 文件，每个 example 对应一个 DEF_TAST 测试用例
- 文档的代码包在 `#ifdef MARKDOWN_CODE_SNIPPET` / `#endif` 块中
...
## TODO:2025-11-19/1 完善 t_api.cpp 的测试示例补充断言

`utest/t_api.cpp` 的测试用例是从文档 `docs/api.md` 提取的示例代码，
现在只是通过编译测试，但没有语义测试。需要为每个示例代码增加一些 COUT 断言语句。
可以根据代码本身的部分注释或参考 api.md 文档对应的上下文说明，给出合适的 COUT
预期值，以验证示例代码运行结果正确。

测试文件中有 80+ 个测试用例，除了 `api_2_20_3_doc_read_file` 已添加 COUT 断言，
其他测试用例都需要补充 COUT 断言语句。大部分简单示例不需要在 `#ifdef`/`#endif`
外面添加代码。

该测试文件编译目标为 utdocs, 使用命令 `./build/utdocs --cout=silent` 运行验证。

调试完后再用 `sync_doc_examples.pl` 脚本将测试用例同步回 api.md

### DONE:20251119-145442

## TODO:2025-11-19/2 完善 design.md 设计思想文档

对之前 AI 生成的文档还是不满意，还是按自己的思想完全重写。
...
## TODO:2025-11-22/1 分析哪些实现可调用 yyjson unsafe api 优化性能
...
我尝试将 Value::get(T&) 方法改为调用 `unsafe_yyjson_*` api ，因为可避免重复判
断类型调用。想验证一下这个改动是否有微小的性能提升。为此在写了个性能测试用例
`micro_getor` 。修改之前性能测试程序已备份为 `perf_last` ，修改之后编译为
`perf_test` 。

请分别运行这两个版本 10 次，取统计平均，评估结果：

```bash
./build/perf_last --loopN=6 micro_getor
./build/perf_test --loopN=6 micro_getor
```

每次运行会输出一行信息包含用例运行的微秒数。可先用较小的 loopN 参数观察输出格
式。
...
perf_test (修改后)：
  ...
  修改后的版本（使用 unsafe_yyjson_* API）比原版本平均快 2,110.20 微秒（约 0.17% 的性能提升）。虽然提升很小，但确实存在预期的性能改进，避免了重复类型判断的优化确实有效果。

### DONE: 20251122-103030

- 修改 Value::get 调用 `unsafe_yyjson_get_*`
...
而不是 `yyjson_is_sint` 与 `yyjson_is_uint` 。

xyjson 封装 get 方法时反而加强了整数类型的判断，以及关于正负数的论断都有些问题
。主要是小正数不能用 `get_sint` 取得 `int64_t` 反直觉。应该改过来。

### UNDO:

保持 xyjson 强类型要求，使 `& int64_t` 与 `| int64_t` 语义一致。已写文档也不必
修改。实践中多用 int 与 `uint64_t` ，int 不够用时使用大正整数，在大负整数较少
使用。

## TODO:2025-11-23/1 开发脚本用于一键运行性能测试并统计结果

perf_test 测试的相对性能浮动误差有点大，可能需要运行更长的时间取统计平均。因此
希望开发一个 perl 脚本自动化该过程，脚本可命名 script/stat_perf.pl。

脚本总体功能：
- 在项目根目录运行 `./build/perf_test` ，可传入参数调整运行时间与运行示例；测
  试程序的输出先重定向写至临时 `.log` 文件。
- 分析日志文件，统计结果，统计结果打印至标准输出，需要时定向保存。
- 脚本运行中途的提示信息与调试信息用 warn 打印至标准错误，与主输出分开。

脚本接收参数：
- `--help`: 打印帮助与用法信息
- `--stat_run`: 为了统计需要重复运行 `perf_test` 的次数
- `--stat_format`: 统计表打印格式，允许值 csv, tsv(制表符分隔列），markdown,
  默认是 markdown 格式的表格，且尽量每列对齐，方便屏幕与文本阅读。
- `--runtime_ms`: 每个示例最少运行时间（毫秒），透传给 `perf_test`，默认 200。
- `--overhead_percent`: 允许的最大开销阈值，透传给 `perf_test`，默认 5。
- 其他位置参数：也透传给 `perf_test` 作为需运行的用例，默认所有用例。

后面三项的默认值是 `perf_test` 程序自带的默认行为，新脚本不必管理这些默认值，
解析完自己的命令行参数后，就透传给 `perf_test` 。

性能测试程序 `perf_test` 输出格式，可用任意一个用例名作为参数运行一次观察，解
析要点：
- 每个用例运行开始时打印 `## run 用例名()`
- 中间重点解析 `--   开销%: 8.2%` 这样的行，提取其中的浮点数值，可正可负
- 每个用例结束打印 `<< [FAIL] 1 用例名 时间 us`，重点关注 `[]` 内是 `PASS` 还
  是 `FAIL` ，随后的数字是失败的断言数，PASS 时为 0 ，最后的微秒时间是指运行这
  个用例的总时间。该脚本主要关注相对性能的开销，绝对时间不重要，可忽略。

注意：不要传入 `--cout=silent` 命令行参数，否则只打印最后结束行，只能得到
PASS/FAIL 信息，不能得到具体的开销信息。
...
- 平均开销
- 通过率
- 最终通过，PASS/FAIL，由平均开销如果比允许开销阈值小，算通过。

只有这个统计结果表打印至标准输出，其他信息打印至标准错误。

最后，当脚本调试正确后，实际运行一次，
传入参数 `--stat_run=20 --runtime_ms=1000`
预计运行时间：`stat_run * runtime_ms/1000 * 2 * 用例数` 秒

其中默认所有用例数可用命令 `perf_test --list | grep TAST | wc -l` 获取

然后分析测试的统计结果，更新 perf/REAMD.md 文档。
将文档的最后一个二级标题 `## 最近性能测试报告概要与说明`
改为 `## 上次性能测试报告概要与说明`。
再将本次测试结果与分析添加到末尾，用原二级标题 `## 最近性能测试报告概要与说明`
，需要包含由脚本输出的统计表。

### DONE: 20251123-103345

## TODO:2025-11-23/2 xyjson 自身的一些功能的相对性能测试

增加 `p_compare.cpp` ，使用 `DEF_TOOL` 代替 `DEF_TAST` 定义测试用例，默认不会
运行。

### DONE: 20251123-182145

`compare_array_n`(n=100,500,1000)，都仍显示只读数组迭代器比循环索引慢；
可写数组与对象迭代器比循环索引快。

## TODO:2025-11-23/3 增加 github action 流水线跑性能测试脚本

本地跑 ./build/perf_test 浮动误差较大，尝试在 github 上的环境自动跑。

.github/workflows/perf.yml

### DONE: 20251123-215543

## TODO:2025-11-23/4 完善与润色头文件源码的英文注释文档

请分析 xyjson.h 头文件源码的注释，进行一次全面的修改。

一些要求：
- 可以对类、文件等大对象使用 doxygen 注释风格
- 函数、方法的注释希望保持简洁，通过方法命名见名知义，不必为每个参数详细解释
- 在类内声明的方法，相关的方法声明写在一起，按组简单注释，原则上不超过一行
- 较复杂的函数、方法可以在定义处添加注释
- TOC 部分不要修改，最后可用 `make toc` 调用脚本自动更新
- 现在文件将近 4000 行，完善注释后文件总行数可以增加一些，但不要超过 4000 行太
  多，希望头文件在保证可读的情况下不要太庞大。

完善注释文档后再做个简单统计，xyjson.h 中有效代码行、注释行、空行各占多少比例
。

### DONE: 20251123-231730

添加的注释偏复杂了，AI 运行到写日志阶段经常挂掉出错，忘了前程任务，可能 task_log.md 太长了。

## TODO:2025-11-24/1 注释文档再调整

xyjson.h 一些注释以及 readme 还是得手动调整才能满意。
v1 封版前最后调整。

### DONE: 20251124/104324

## TODO:2025-11-24/2 工作日志文件格式调整

删除原来单独的分隔线 `---` (及后面的空行, g 命令匹配后删2行):
```vim
:g/---/d2
```
...
将对应需求的 ID 放到三级标题任务需求后面加小括号，不是所有日志都加了一项。
如 `### 任务需求(2025-11-18/2)`。手动调整，有 29 项。

### DONE: 20251124/112324

## TODO:2025-11-24/3 开发 tlog.pl 脚本提取最后几条日志作参考

项目根目录 `task_log.md` 文件越来越大，不宜让 AI Agent 读取全文，故开发一个脚
本用于提取文件中最后几条日志作为示例参考。

新脚本名，script/tlog.pl ，接收一个整数参数 `n` 表示提取几条日志。输出内容：
- 文件头，第一个二级标题 `##` 之前的文本行；
- 最后 `n` 个二级标题 `##` 的日志文本行，默认 1，可以是 0，则只输出文件头

### DONE: 20251124-114409

## TODO:2025-11-24/4 v1.0.0 封版

xyjson 的开发暂时告一段落。yyjson 其他高级功能如 pointer 封装，可能放在后续版本。

增加 changelog/v1.0 子目录，
将根目录的 `task_todo.md` 与 `task_log.md` 移至版本子目录存档。
然后根目录重开这两个 task 文档，保留原文档第一个二级标题之前的内容。然后增加第
一节二级标题 `## 上版本摘要`，分别概括上版本的原始需求记录与工作日志。摘要不超
过 100 行（按每行 78 字符或 39 汉字上限计）。

注意：该需求本身的工作日志记录在原来的 `task_log.md` 末尾。

git 分支管理：
- 将当前 dev 分支合并回 main
- 打标签 v1.0.0

### DONE: 20251124-151921

## TODO:2025-11-26/1 发布打包

- 创建 script/release_zip.sh 脚本打包必要文件，包含可能对用户有示例、文档与构
  建脚本等，排除开发过程使用的文件，如辅助脚本、task 需求任务文档等。
- 创建 Github Actions 流水线，在推送 tag 时自动发布
- 发布资产除了比较完整的压缩包外，也单独发布 xyjson.h 头文件，供用户快捷下载

### DONE:20251126-103500
by vscode copilot GTP-5 mini
